var moves = [];
var move_pos = document.createElement('div');
move_pos.id = "move_ps"
document.body.appendChild(move_pos);
(function (global) {
    ...
    function createHexagonal5Board() {
        var board = new HexGridBoard(7, 7);
        board.fillRect(2, 0, 4, 1, true);
        board.fillRect(1, 1, 5, 1, true);
        board.fillRect(1, 2, 6, 1, true);
        board.fillRect(0, 3, 7, 1, true);
        board.fillRect(1, 4, 6, 1, true);
        board.fillRect(1, 5, 5, 1, true);
        board.fillRect(2, 6, 4, 1, true);
        board.pullPeg(board.xy(3, 3));
        return board;
    }
    ...
    function History() {
        this.add = function (from, to) {
            ...
            var move_pos_ch = document.createElement('div');
            move_pos_ch.innerHTML = `( ${from} , ${to} )`;
            move_pos.appendChild(move_pos_ch);
        };
        this.undo = function (board) {
            if (moves.length > 0) {
                var lastMove = moves.pop();
                board.undoMovePeg(lastMove.from, lastMove.to);
                var remove_move_pos = document.querySelector('#move_ps>div:last-of-type');
                move_pos.removeChild(remove_move_pos);
            }
        };
